@php
    if(isset($expense_category_id)) {
        if($expense_category_id != "all"){
            $expenses = App\Expense::where('school_id', school_id())->where('session', get_schools())->where('date', '>=', $date_from)->where('date', '<=', $date_to)->where('expense_category_id', $expense_category_id)->get();
        }else {
            $expenses = App\Expense::where('school_id', school_id())->where('session', get_schools())->where('date', '>=', $date_from)->where('date', '<=', $date_to)->get();
        }
    }else {
        $expenses = App\Expense::where('school_id', school_id())->where('session', get_schools())->where('date', '>=', $date_from)->where('date', '<=', $date_to)->get();
    }
